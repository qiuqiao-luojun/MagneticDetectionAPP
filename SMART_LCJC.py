...
class QmyMainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.__labMove.setStyleSheet('font:bold 20px;')
        self.ui.statusbar.addWidget(self.__labMove)

        self.__labPick = QLabel("提示信息栏")
        self.__labPick.setMinimumWidth(1300)
        self.__labPick.setStyleSheet('font:bold 20px;')
    ...
    def __iniFigure(self):  ##创建绘图系统，初始化窗口
        ...
        self.ui.statusbar.setFixedHeight(40)
    ...
    def do_canvas_mouseMove(self, event):
        if event.inaxes == None:
            return
        info = "%s 缺陷位置=%.3fm  缺陷幅值=%.1f%% (门限为%.1f%%)" % (
        event.inaxes.get_label(), self.x_encode_data[int(event.xdata)], (event.ydata - ylim_min) / (4096 - ylim_min) * 100, AlarmThreshold1)
    ...
    def __dispalyFigure(self):  ## 绘图
        try:
            ...
            for i in leg.legendHandles:
                i.set_linewidth(12)
            self.__fig.canvas.draw()  # 重绘
            self.__labPick.setText("数据加载完毕!\t文件打开地址：" + self.__filename)
        except Exception as err:
            print(traceback.format_exc())
